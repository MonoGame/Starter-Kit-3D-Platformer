...
public class PlatformerGame : Game
{
    ...
    protected override void LoadContent()
    {
        ...
        _logoTexture = Content.Load<Texture2D>(Path.Combine("Textures", "logo"));
        _coinTexture = Content.Load<Texture2D>(Path.Combine("Textures", "coin"));
        _foundationTexture = Content.Load<Texture2D>(Path.Combine("Textures", "foundation"));
        _font = Content.Load<SpriteFont>(Path.Combine("Font", "hud"));
        _debugFont = Content.Load<SpriteFont>(Path.Combine("Font", "debug"));
        Content.Load<Model>(Path.Combine("Models", "platform-large"));
        Content.Load<Model>(Path.Combine("Models", "cloud"));
        Content.Load<Model>(Path.Combine("Models", "character"));
        ...
        _song = Content.Load<SoundEffect>(Path.Combine("Sounds", "bright")).CreateInstance();
        _song.IsLooped = true;
        _song.Volume = 0.0f;
#if !DEVMODE
        _song.Play();
        ...
        {
            _mainMenu = new Menu(_font, Content, null, MenuTransitionDirection.Right);
        }
        ...
        var playerLoading = new AnimatedEntity(Content.Load<Model>(Path.Combine("Models", "character")), Content)
        {
